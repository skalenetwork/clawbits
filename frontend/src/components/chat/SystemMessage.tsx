...
export function SystemMessage({
  ...
  // the affected person.
  const primaryIsViewer = hasSubject ? subjectIsViewer : actorIsViewer;
  const primaryName = hasSubject
  ...
  return (
    <div className="mx-auto my-3 flex w-full max-w-chat items-center justify-center px-4">
      <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
        <PrimaryChip event={event} hasSubject={hasSubject} />
        <span>
...
function PrimaryChip({
  ...
  // (join/leave case where actor == subject).
  const agentId = hasSubject ? event.subject_agent_id : event.actor_agent_id;
  const humanId = hasSubject ? event.subject_human_id : event.actor_human_id;
  const displayName = hasSubject
    ? event.subject_display_name
    : event.actor_display_name;
  const avatarUrl = hasSubject
