...
export function buildHermesSetupPrompt(org: Org | null, signupToken: string): string {
    ...
    return [
        ...
        "fi",
        "cd \"$REPO\"",
        "if ! grep -q -- '--restart-default' extensions/hermes/reinstall.sh; then",
        "  echo 'This Clawbits checkout is too old. Ask the operator to update it (git pull), then rerun.'",
        "  exit 1",
        "fi",
        "",
        "# 2. Install or upgrade the plugin in the active Hermes profile and exchange this",
        "#    one-time token for the agent's API key. This is non-destructive: an existing",
        "#    install is upgraded in place and keeps its identity, settings and queued work.",
        "#    The script restarts only this profile's gateway and waits until it is healthy.",
        `./extensions/hermes/reinstall.sh --endpoint ${JSON.stringify(apiBase)} --signup-token ${JSON.stringify(signupToken)}`,
        "",
        "# 3. Exit code 4 means this profile already runs a Clawbits agent and the token was",
        "#    NOT used. Stop and ask the operator which they want: enroll in a new Hermes",
        "#    profile (`hermes profile create NAME`, then rerun step 2 with `--profile NAME`),",
        "#    or replace this profile's agent (rerun step 2 with `--reset -y`). Do not pick one yourself.",
        "#    Exit code 3 means the profile is served by the default Hermes gateway, and applying the",
        "#    install restarts every profile it serves. Ask the operator; only if they agree, rerun",
        "#    step 2 with `--restart-default` and without `--signup-token` (after `--reset`, show the",
        "#    operator the script's output instead).",
        "#    Any other non-zero exit: show the operator the script's output.",
        "",
        "# 4. Verify (with --profile NAME: `hermes -p NAME clawbits doctor`). Exit 0 is healthy,",
        "#    1 degraded, 3 not ready; report anything but 0 to the operator.",
        "hermes clawbits doctor",
    ].join("\n");
}
