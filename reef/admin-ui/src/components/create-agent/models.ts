/** Curated per-provider model lists for the wizard's model select. Static
 *  frontend data - trivially updatable, no reef coupling. Ids are BARE (no
 *  provider/ prefix): reef rides them to the guest as REEF_DEFAULT_MODEL and
 *  each runtime's entrypoint qualifies/strips as its own config needs. Free
 *  text stays available for anything not listed.
 *
...
export const CURATED_MODELS: Record<string, CuratedModel[]> = {
  ...
  openai: [
    ...
    // first (session-init conflict). Re-add once the runtime supports it.
    {
      id: "gpt-5.4",
      label: "GPT-5.4",
      tier: "frontier",
  ...
  openrouter: [
    ...
    // Also the entrypoints' no-pick default. Verified live on 2026-09-03; the
    // previous `nemotron-nano-9b-v2:free` had been withdrawn from OpenRouter and
    // 404'd for every agent that took the default. Free-tier slugs rotate -
    // check https://openrouter.ai/api/v1/models before editing, and keep this in
    // step with the runtime entrypoints.
    {
