...
This starts Vite (from `../frontend`) and opens a native window pointing at it. Rust deps compile on first run (1 to 2 min cold).
...
The Linux build auto-merges [`src-tauri/tauri.linux.conf.json`](src-tauri/tauri.linux.conf.json): its window entry replaces the macOS one wholesale (so an opaque window with native decorations; GTK has no vibrancy), it pulls in `libnotify4`, and it applies the custom [`clawbits.desktop`](src-tauri/clawbits.desktop) template (adds `GenericName`, `Keywords`, `StartupWMClass`, `MimeType`).
...
- **.deb users**: the updater tells them a new version exists, but the install step fails silently; they need to download the next `.deb` manually (or you can publish to a real apt repo).
...
- **App icon + description** in GNOME Software / app drawer search ("clawbits-staging"): the icon shows, `GenericName` shows "AI Agent Messaging (Staging)", search terms from `Keywords=` work.
...
- `notify-send -a clawbits-staging "test" "from notify-send"`: if this works but our app doesn't, our hint set is the problem; if neither works, the daemon or desktop env is.
...
- `update-desktop-database ~/.local/share/applications` is sometimes needed after install before the Shell sees the file.
