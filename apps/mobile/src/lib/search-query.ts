/**
 * Parse search-operator syntax out of a search query and resolve it to
 * structured message-search filters. Names (`in:`, `from:`) are resolved to ids
 * against the already-loaded channel / member / agent lists, so the server
 * receives ids, not fuzzy names (see docs/protocol/SEARCH_SPEC.md §8). Ported
 * from `frontend/src/lib/searchQuery.ts`; the only divergence is the matcher
 * (mobile uses the lightweight `nameMatchScore` rather than the web's fuzzy
 * scorer) and the single-arg `formatChannelTitle`.
 *
...
import { nameMatchScore, RESOLVE_MIN } from '@/lib/fuzzy';
...
function resolveChannel(value: string, channels: MmChannel[]): MmChannel | null {
  ...
  for (const c of channels) {
    ...
    const score = Math.max(nameMatchScore(v, c.name), nameMatchScore(v, title));
...
function resolveAuthor(
  ...
  for (const m of members) {
    const score = Math.max(nameMatchScore(v, m.display_name ?? ''), nameMatchScore(v, m.email));
    if (score >= RESOLVE_MIN && (!human || score > human.score)) human = { m, score };
  }
  let agent: { a: AgentUser; score: number } | null = null;
  for (const a of agents) {
    const score = Math.max(
      nameMatchScore(v, a.display_name ?? ''),
      nameMatchScore(v, a.nickname ?? ''),
      nameMatchScore(v, a.agent_id),
    );
...
export function parseSearchQuery(raw: string, sources: SearchSources): ParsedQuery {
    ...
    (match: string, lead: string, opRaw: string, full: string, quoted?: string) => {
      ...
      switch (op) {
        case 'in': {
          ...
          chip(`in ${formatChannelTitle(c.display_name ?? c.name)}`);
          return lead;
        }
        case 'from': {
  ...
  return { text: text.replace(/\s+/gu, ' ').trim(), filters, chips };
}
