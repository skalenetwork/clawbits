...
**Plugins baked in the Dockerfile:** the strict `clawbits-openclaw-plugin` channel plus the `clawbits-openclaw-tools` services/tools companion. The channel resolves first and the companion installs at that exact version; `CLAWBITS_PLUGIN_VERSION` pins both explicitly. The post-build probe rejects a missing or mismatched pair. Both install as the `node` user before the entrypoint layer.

**Install flags are probed, never assumed,** because OpenClaw's install gating has changed three times across the versions we support. The build reads `plugins install --help` once and passes `--acknowledge-clawhub-risk` (pre-2026.8), `--acknowledge-install-policy-warning` (its 2026.8 replacement), and `--accept-capabilities` (new in 2026.8, and required for us: OpenClaw refuses to commit an external plugin's artifact without capability consent, a Docker `RUN` has no TTY to prompt on, and Clawbits is not in OpenClaw's official catalogs so it gets no exemption) only when the CLI recognizes each one. `--pin` is the exception: 2026.8+ rejects it for a `clawhub:` ref (*"--pin is only supported with npm registry installs."*) while the option itself still exists, so `--help` cannot detect it — the channel install therefore tries with `--pin` and retries without, and the companion pins via its `@version` spec instead.
...
3. **Wire the clawbits channel and companion** — the entrypoint sets `channels.clawbits.serviceOwner=tools` on every boot, giving cron/email/usage/skills ownership to the companion before Gateway startup. Account credentials remain under `channels.clawbits.accounts.default.*`, so both packages reuse the same identity and the entrypoint bridges injected `CLAWBITS_*` into that config via `openclaw config set`. Four shapes:
...
- ✅ **Custom guest env, at create AND after** - `POST /fleet` takes an optional `env` map, merged at **lowest precedence** (reef-managed wiring always wins), and `GET`/`PATCH /fleet/{id}/env` view and edit it afterwards (this was create-only before).
  - **Rejected keys (422), on both paths.** Keys reef wires itself and the whole `REEF_*` prefix, plus - since the edit path exists - every key the agent's own profile can inject (`profiles.AgentProfile.managed_env_keys`: `GATEWAY_AUTH_TOKEN`, `SECRETS_MASTER_KEY`, `IRONCLAW_PUBLIC_URL`, the `HERMES_DASHBOARD_*` family, …), and a short dangerous set (`LD_PRELOAD`, `PATH`, `HOME`, `NODE_OPTIONS`, …) that would load code into every child the gateway spawns or brick the guest. `OPENCLAW_*` config knobs and IronClaw's `LLM_BACKEND` stay allowed on purpose. Names/count/per-value size/total size are validated; every message names the key and, at most, a LENGTH.
  - **Reading it back.** `GET /fleet/{id}/env` returns key names + value **lengths** and never a value - reef cannot tell which of a customer's vars is a secret (`redact_env` is a key-NAME heuristic that returns `DATABASE_URL` in full), so all of them are treated as one. There is no reveal endpoint and there must not be one. `GET /fleet/{id}/logs` substitutes the current user-env values with `***` on the read path.
  - **Applying a change.** `PATCH` takes `{set, unset, apply}`. `apply:"restart"` writes a per-agent file on a **read-only** mount (`reef/guest_env.py`; the image entrypoint parses it, never sources it) and restarts in place, which preserves the whole container FS incl. `~/.openclaw`; it needs an image that advertises `REEF_FEATURES=env-file`, and `GET /env` reports what this agent can take as `apply_modes`. `apply:"recreate"` works on every existing agent but destroys `~/.openclaw` (config, sessions, identity) and re-pins the model, exactly like an upgrade. `apply:"none"` writes only. A no-op PATCH restarts nothing, and a deliberately stopped agent stays stopped.
  - **Still never persisted** (the store stays secret-free, §9 #4): values live in the container env - value-masked for secret-named keys in fleet detail - and in that per-agent file on the reef host, never in reef's DB or its backups. Anything set here is readable by the agent and by anyone with its web terminal (`/proc/1/environ`): use scoped, revocable credentials.
  - Surfaced as a collapsed "env variables" editor in both create dialogs and as an edit panel in the reef admin-ui + the clawbits reef page (both gated on the `/providers` `features` flags `env` / `env-edit`, so an older reef can't silently drop the field).
- ✅ **Per-agent capabilities** — `POST /fleet` takes an optional `capabilities` list (`reef/capabilities.py`), persisted on the record and carried into the guest as `REEF_CAPS` for the entrypoint to act on. The list is short on purpose: only things the **microVM does not contain** are gated (arbitrary shell, package installs and the browser are all ungated, because the VM *is* the boundary). Today: **`gh`** — links the staged GitHub CLI onto `PATH`; **`cron`** — the self-scheduling tool plus `cron.enabled`, so a grant and a revoke both actually land. **Omitting the field applies `DEFAULT_CAPABILITIES` (`gh`); an explicit `[]` grants nothing** — the two are deliberately different, and both wizards send the field even when empty so an unticked box is honored. `gh` defaults on because a second gate sits under it (reef injects **no** GitHub token, so the CLI is inert until a human supplies one); `cron` does not, so it stays a deliberate act. Unknown names are a **422** rather than being dropped, and every toggle is a `REEF_*` var — a reserved prefix — so an agent cannot self-grant one through the `env` passthrough. `PATCH /fleet/{id}` re-grants or revokes, but the guest reads `REEF_CAPS` **once at boot**: a change lands on the agent's next upgrade/recreate, not immediately, and changing the defaults never retroactively widens an existing agent's grant.
