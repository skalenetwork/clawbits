...
**Plugin baked in the Dockerfile:** `openclaw plugins install clawhub:clawbits-openclaw-plugin --pin --acknowledge-clawhub-risk`. Runs as the `node` user → lands under the user plugin root; placed *before* the entrypoint `COPY` so editing the entrypoint doesn't bust the network layer. `--pin` locks the version (reproducible; bump deliberately, like the base tag). `--acknowledge-clawhub-risk` is mandatory non-interactively: ClawHub's scan flags this first-party plugin as `suspicious` (high local blast radius, not malicious), and newer openclaw CLIs abort a silent install without it.

**Plugin allowlist:** the entrypoint pins `plugins.allow=["clawbits"]` (`openclaw config set plugins.allow … --json`) on every boot. OpenClaw warns when `plugins.allow` is empty while a non-bundled plugin (our baked-in `clawbits`, id from its manifest) is discoverable (*"may auto-load; set trusted ids"*); pinning the allowlist to the single plugin we ship silences that and enforces an explicit trusted inventory — only `clawbits` is eligible to load (bundled chat channels still activate when their channel is enabled). Idempotent, so it survives a persisted-config restart.
...
6. **Scoped web terminal (§13).** When Reef sets `REEF_TERMINAL_ENABLE=1` (+ `REEF_TERMINAL_PASSWORD` — the **same** secret as the gateway token — + `REEF_TERMINAL_PORT`), the entrypoint launches **`ttyd`** on `:7681` serving a narrow `openclaw`-only shell (`reef-term.sh` → `reef-term.mjs`), backgrounded so the gateway stays the foreground/liveness process. `REEF_TERMINAL_SHELL=full` swaps in a real shell.
