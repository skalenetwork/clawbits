...
**Plugins baked in the Dockerfile:** the strict `clawbits-openclaw-plugin` channel plus the `clawbits-openclaw-tools` services/tools companion. The channel resolves first and the companion installs at that exact version; `CLAWBITS_PLUGIN_VERSION` pins both explicitly. The post-build probe rejects a missing or mismatched pair. Both install as the `node` user before the entrypoint layer. Because the ClawHub trust flag exists only in some supported OpenClaw releases, the build probes `plugins install --help` and passes `--acknowledge-clawhub-risk` only when recognized.
